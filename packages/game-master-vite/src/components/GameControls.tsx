...
const AI_PROVIDERS = [
  { id: 'openrouter', name: 'OpenRouter', description: '支持多种模型 (Claude, GPT, Gemini 等)', defaultModel: 'anthropic/claude-3-haiku', keyPlaceholder: 'sk-or-...', baseURL: 'https://openrouter.ai/api/v1' },
  { id: 'openai', name: 'OpenAI', description: 'GPT-4, GPT-3.5 等模型', defaultModel: 'gpt-4', keyPlaceholder: 'sk-...', baseURL: '' },
  { id: 'minimax', name: 'MiniMax', description: 'MiniMax-M2 模型', defaultModel: 'MiniMax-M2', keyPlaceholder: 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...', baseURL: 'https://api.minimaxi.com/v1' },
  { id: 'custom', name: '自定义', description: 'OpenAI 兼容格式 API', defaultModel: '', keyPlaceholder: 'your-api-key', baseURL: '' }
];
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      try {
        ...
        const effectiveBaseURL = provider === 'custom' ? baseURL.trim() : (selectedProvider?.baseURL || '');
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {showApiKeyConfig && (
          <div className="border rounded-lg p-4 space-y-4 bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">AI 服务配置</h3>
            </div>
              ...
              {provider === 'custom' && (
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    输入 OpenAI 兼容格式的 API 地址，如: https://api.example.com/v1
                  </p>
                </div>
              )}
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                {provider !== 'custom' && (
                  <p>
                    💡 获取 API Key：
                    {provider === 'openrouter' && (
                      <a href="https://openrouter.ai/keys" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline ml-1">
                        OpenRouter
                      </a>
                    )}
                    {provider === 'openai' && (
                      <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline ml-1">
                        OpenAI
                      </a>
                    )}
                    {provider === 'minimax' && (
                      <a href="https://platform.minimaxi.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline ml-1">
                        MiniMax
                      </a>
                    )}
                  </p>
