...
- **AI 服务**:
  - OpenRouter (支持多种模型: Claude, GPT, Gemini 等)
...
- **Docker Compose** 1.29+
- **AI API Key** (任选其一):
  - **OpenRouter API Key** (推荐): 从 [OpenRouter](https://openrouter.ai) 获取，支持多种模型
...
2. 点击 **"🔑 配置 API Key"** 输入您的 AI API Key
   - 支持 OpenRouter、OpenAI、MiniMax API Key
   - 系统会自动识别 Key 类型并配置对应的 provider
...
在 `packages/player/configs/` 目录下的 JSON 配置文件中设置：
...
支持的 provider 值：
- `openrouter` - OpenRouter 平台（推荐，支持 Claude、GPT、Gemini 等）
- `openai` - OpenAI 官方 API
- `minimax` - MiniMax AI

#### 方式三：通过前端界面配置

在游戏界面中点击 **"🔑 配置 API Key"** 按钮，直接输入 API Key，系统会自动识别并配置。
